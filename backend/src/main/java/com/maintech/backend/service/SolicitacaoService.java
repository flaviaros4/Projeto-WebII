...
public class SolicitacaoService {
    ...
    public Solicitacao aprovarOrcamento(Long solicitacaoId) {
        ...
        Solicitacao salva = solicitacaoRepository.save(solicitacao);
        adicionarHistorico(salva, estadoAnterior, EstadoSolicitacao.APROVADA, "Orçamento aprovado pelo cliente");
        return salva;
    }
    ...
    public Solicitacao efetuarManutencao(Long solicitacaoId, ManutencaoRequest request) {
        ...
        solicitacao.setEstado(EstadoSolicitacao.ARRUMADA);
    ...
    public Solicitacao rejeitarOrcamento(Long solicitacaoId, String motivo) {
        ...
        Solicitacao salva = solicitacaoRepository.save(solicitacao);
        adicionarHistorico(salva, estadoAnterior, EstadoSolicitacao.REJEITADA, "Orçamento rejeitado. Motivo: " + motivo);
        return salva;
    }
    ...
    public Solicitacao resgatarServico(Long solicitacaoId) {
        ...
        Solicitacao salva = solicitacaoRepository.save(solicitacao);
        adicionarHistorico(salva, estadoAnterior, EstadoSolicitacao.APROVADA, "Serviço resgatado pelo cliente");
        return salva;
    }
    ...
    public Solicitacao pagarServico(Long solicitacaoId) {
        ...
        Solicitacao salva = solicitacaoRepository.save(solicitacao);
        adicionarHistorico(salva, estadoAnterior, EstadoSolicitacao.PAGA, "Serviço pago pelo cliente");
        return salva;
    }

    // --- NOVO MÉTODO RF016 ---
    public Solicitacao finalizarSolicitacao(Long solicitacaoId) {
        Solicitacao solicitacao = solicitacaoRepository.findById(solicitacaoId)
                .orElseThrow(() -> new RuntimeException("Solicitação não encontrada"));

        Usuario usuario = usuarioService.getUsuarioAtual();
        if (!(usuario instanceof Funcionario)) {
            throw new RegraNegocioException("Apenas funcionários podem finalizar solicitações.");
        }
        Funcionario funcionario = (Funcionario) usuario;

        if (solicitacao.getEstado() != EstadoSolicitacao.PAGA) {
            throw new RegraNegocioException("Só é possível finalizar solicitações que já foram pagas.");
        }

        EstadoSolicitacao estadoAnterior = solicitacao.getEstado();
        solicitacao.setEstado(EstadoSolicitacao.FINALIZADA);
        solicitacao.setDataHoraFinalizacao(LocalDateTime.now());
        solicitacao.setFuncionarioFinalizacao(funcionario);

        Solicitacao salva = solicitacaoRepository.save(solicitacao);
        adicionarHistorico(salva, estadoAnterior, EstadoSolicitacao.FINALIZADA, "Solicitação finalizada por " + funcionario.getNome());
